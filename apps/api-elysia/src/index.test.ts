...
describe('app smoke (no DB)', () => {
  it('answers /v1/ping', async () => {
    const res = await app.handle(new Request('http://localhost/v1/ping'));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ message: 'pong from elysia' });
  });
  ...
  it('issues a CSRF token plus cookie', async () => {
    const res = await app.handle(new Request('http://localhost/v1/csrf-token'));
    expect(res.status).toBe(200);
  ...
  it('login reports class-validator style messages', async () => {
    ...
    const tokenRes = await app.handle(new Request('http://localhost/v1/csrf-token'));
