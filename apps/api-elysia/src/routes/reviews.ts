...
type ReviewRow = Review & { user: User };
...
async function ownedReview(reviewID: string, userID: string): Promise<ReviewRow> {
...
async function createReview(userID: string, slug: string, rating: number, comment: string | null) {
  ...
  await recomputeRating(p.id);
  return toDTO(review, p.slug);
}

export const reviews = new Elysia()
  .post('/products/:slug/reviews', async ({ request, cookie, params, body, set }) => {
    const auth = await requireAuth({ request, cookie });
    const in1 = bindBody<{ rating?: number; comment?: string | null }>(
  ...
  .get('/products/:slug/reviews', async ({ params, query }) => {
    const slug = params.slug!;
    if (!(VALID_SLUGS as readonly string[]).includes(slug)) {
      throw new ApiError(404, `Product with slug ${slug} not found`);
    }
    const { page, limit } = paging(query.page, query.limit, 50);
    const p = await db().product.findFirst({ where: { slug: slug as ProductSlug } });
    if (p === null) throw new ApiError(404, `Product with slug ${slug} not found`);
  ...
  .patch('/reviews/:id', async ({ request, cookie, body, params }) => {
    const auth = await requireAuth({ request, cookie });
    const in1 = bindBody<{ rating?: number; comment?: string | null }>(
      body as Record<string, unknown>,
      {
    ...
    const review = await ownedReview(params.id!, auth.user.id);
  ...
  .delete('/reviews/:id', async ({ request, cookie, params }) => {
    const auth = await requireAuth({ request, cookie });
    const review = await ownedReview(params.id!, auth.user.id);
