...
export const blog = new Elysia()
  .get('/blog/posts', async ({ query }) => {
    // ponytail: Go blog paging reads `pageSize`, not `limit`.
    const { page, limit } = paging(query.page, query.pageSize, 100);
  ...
  .get('/blog/posts/search', async ({ query }) => {
    const q = query.q;
  ...
  .get('/blog/posts/admin/all', async ({ request, cookie, query }) => {
  ...
  .post('/blog/posts', async ({ request, cookie, body, set }) => {
  ...
  .patch('/blog/posts/:id', async ({ request, cookie, body, params }) => {
    await requireAdmin({ request, cookie });
    const p = await fetchPost('id', params.id!);
    const in1 = bindBody<{
      title?: string | null;
      content?: string | null;
      excerpt?: string | null;
  ...
  .delete('/blog/posts/:id', async ({ request, cookie, params }) => {
    await requireAdmin({ request, cookie });
    const p = await fetchPost('id', params.id!);
  ...
  .get('/blog/posts/:slug', async ({ params, query }) => {
    const p = await fetchPost('slug', params.slug!);
