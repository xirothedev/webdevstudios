...
export const usersRoutes = [
  route('GET', '/users/me', async (ctx) => {
    const auth = await requireAuth(ctx);
    return privateDTO(auth.user);
  }),
  route('PATCH', '/users/profile', async (ctx) => {
    const auth = await requireAuth(ctx);
  ...
  route('PATCH', '/users/avatar', async (ctx) => {
    const auth = await requireAuth(ctx);
    if (!storageEnabled()) {
  ...
  route('GET', '/users/:id', async (ctx) => {
    ...
    const u = await db().user.findUnique({ where: { id: ctx.params.id! } });
    if (u === null) {
