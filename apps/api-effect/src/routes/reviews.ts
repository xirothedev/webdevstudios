...
export const reviewsRoutes = [
  route('POST', '/products/:slug/reviews', async (ctx) => {
  ...
  route('GET', '/products/:slug/reviews', async (ctx) => {
  ...
  route('PATCH', '/reviews/:id', async (ctx) => {
    const auth = await requireAuth(ctx);
    const in1 = bindBody<{ rating?: number; comment?: string | null }>(await bodyOf(ctx), {
      Rating: { type: 'number', integer: true },
      Comment: { type: 'string' },
    });
    const review = await ownedReview(ctx.params.id!, auth.user.id);
  ...
  route('DELETE', '/reviews/:id', async (ctx) => {
    const auth = await requireAuth(ctx);
    const review = await ownedReview(ctx.params.id!, auth.user.id);
